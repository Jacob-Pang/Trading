    ...
    def open_position(self, entry_price: float, size: float, transact_fee_rate: float,
        ...
        if as_contra_position:
            ...
            if contra_position > 0: # Long contra_position to short existing_position
                end_balances.add_balance(self.base, abs(size)) # Invert from negative to positive
                end_balances.add_balance(self.quote, -(entry_price * contra_position))
            else: # Short contra_position to long existing_position
                end_balances.add_balance(self.base, contra_position)
                end_balances.add_balance(self.quote, -(1. - transact_fee_rate) * (entry_price * contra_position))
        elif size > 0: # Long position: transaction fee incurred against base currency
            end_balances.add_balance(self.base, (1. - transact_fee_rate) * size)
            end_balances.add_balance(self.quote, -(entry_price * size))
        else: # Short position: transaction fee incurred against quote currency
            end_balances.add_balance(self.base, size)
